...
on("directive.init", ({ el, directive: directive2, cleanup, component }) => {
  if (["snapshot", "effects", "model", "init", "loading", "poll", "ignore", "id", "data", "key", "target", "dirty", "scroll"].includes(directive2.value))
    return;
  ...
  if (!directive2.expression || directive2.expression.trim() === "")
    return;
