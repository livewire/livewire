    ...
    function onMutate(mutations) {
      ...
      for (let i = 0; i < mutations.length; i++) {
        ...
        if (mutations[i].type === "attributes") {
          ...
          if (el.textContent.includes("Anonymous Form Test")) {
            console.log("here baby", el, mutations[i]);
          }
      ...
      for (let node of addedNodes) {
        ...
        if (node.textContent.includes("Anonymous Form Test")) {
          console.log("picked up by mutation observer");
        }
...
directive("confirm", ({ el, directive: directive2 }) => {
  let message2 = directive2.expression;
  ...
  el.__livewire_confirm = (action) => {
    if (shouldPrompt) {
      let [question, expected] = message2.split("|");
      if (!expected) {
