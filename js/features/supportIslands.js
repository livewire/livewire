...
export function initSupportIslands() {
    if (!window.Alpine) {
        console.warn(
            "Alpine.js is required for Livewire islands to work properly"
        );
        return;
    }

    window.Livewire.on("effects", (effects, component) => {
        if (effects.islands) {
            effects.islands.forEach((island) => {
                renderIsland(component, island.name, island.content);
            });
        }
    });

    window.Livewire.hook("effects", (effects, component) => {
        if (effects.islands) {
            effects.islands.forEach((island) => {
                renderIsland(component, island.name, island.content);
            });
...
export function renderIsland(component, name, content) {
    ...
    // Find island start marker
    const startMarkerText = `<!--[if ISLAND:${name}`;
    const endMarkerText = `<!--[if ENDISLAND:${name}]><![endif]-->`;

    let startMarker = null;
    let endMarker = null;

    // Walk through all comment nodes to find markers
    ...
    while ((node = walker.nextNode())) {
        if (node.nodeValue.startsWith(`[if ISLAND:${name}`)) {
            startMarker = node;
        } else if (node.nodeValue === `[if ENDISLAND:${name}]><![endif]`) {
            endMarker = node;
            break;
        }
    }
    ...
    if (!startMarker || !endMarker) {
        console.warn(`Island markers for "${name}" not found`);
        return;
    }

    // Get mode from start marker (append, prepend, or replace)
    const mode = startMarker.nodeValue.includes(":append")
        ? "append"
        : startMarker.nodeValue.includes(":prepend")
        ? "prepend"
        : "replace";

    // Get all nodes between markers
    ...
    if (mode === "append") {
        ...
        while (tempDiv.firstChild) {
            endMarker.parentNode.insertBefore(tempDiv.firstChild, endMarker);
        }
    } else if (mode === "prepend") {
        // Add new content before existing content
        const firstNode = nodesToReplace[0] || endMarker;
        while (tempDiv.lastChild) {
            firstNode.parentNode.insertBefore(tempDiv.lastChild, firstNode);
        }
    } else {
