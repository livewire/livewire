...
function supportEntangleV3() {
    window.addEventListener('alpine:rename-me', ({ target: el, detail: { rawData, reactiveData } }) => {
            ...
            Object.entries(rawData).forEach(
                ([key, value]) => {
                    if (
                        !!value &&
                        typeof value === 'object' &&
                        value.livewireEntangle && typeof value.livewireEntangle === 'string'
                    ) {
                        // Ok, it looks like someone set an Alpine property to $wire.entangle or @entangle.
                        let livewireProperty = value.livewireEntangle
                        let isDeferred = value.isDeferred
                        ...
                        livewireComponent.watch(
                            ...
                            value => {
                                // Ensure data is deep cloned otherwise Alpine mutates Livewire data
                                reactiveData[key] = typeof value !== 'undefined' ? JSON.parse(JSON.stringify(value)) : value
                            }
                        )
                    }
                }
...
function isV3() {
    return /^3\..+\..+$/.test(window.Alpine.version)
}
