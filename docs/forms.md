Forms are the most common and critical need in web applications. One of Livewire's primary strengths is making them easy for developers to build, and pleasant for your users to use. Livewire has lots of utiltities to make next-level interface requirements like real-time input validation extremely simple. Let's dive in.

## Submitting a form

Let's start by looking at the most basic form, in a `CreatePost` component. This form will have two simple text inputs and submit button, and then some code on the backend to manage that state and submission.
...
class CreatePost extend Component
{
    public $title = '';
...
To avoid storing incomplete or dangerous use input, most forms need some sort of input validation.

Livewire makes validating your forms simple by adding `#[Rule]` attributes above the properties you want validated.
...
Let's add some basic validation rules to the `$title` and `$content` property in our `CreatePosts` component:
...
There's a lot more you can do with validation in Livewire. For more information, visit the [dedicated documentation page for Validation](/docs/validation).

### Extracting into a form object

If you are working with a large form and prefer to extract all of it's properties, validation logic, etc. into a seperate class, Livewire offers: form object.

Form Objects are a simple abstraction to allow you to re-use form logic across components as well as just provide a nice way to keep your compoent class cleaner and group all form related code into a seperate class.
...
class UpdatePost extend Component
{
    public PostForm $form;
...
As you can see, we've added a `setPost` method to the `PostForm` object to optionally allow for filling the form with existing data as well as storing the post on the object for later use. We've also added a `update()` method for updating the existing post.
...
By default, Livewire only sends a network request when the form is submited (or any other [action](/docs/actions) is called), not while the form is being filled out.
...
Now, as a user types into this field, network requests will be sent to the server to update `$title`. This is useful for things like a real-time search, where a dataset is being filtered as a user types into a search box.

## Only updating fields on "blur"

For most cases, `wire:model.live` is fine for real-time form field updating, however, it can be a little aggressive on text inputs.
...
Sometimes, you might want to show validation errors as the user fills out the form. This way, they  are alerted early that something is wrong, instead of having to wait until the entire form is filled out.
...
Now, if the user only types "3" characters into the "title" input, then clicks onto the next input in the form, a validation message will be shown to them indicating there is a 5 character minimum for that field.

[You can read more about validation in Livewire here.](/docs/validation)

## Real-time form saving

If you want to automatically save a form as the user goes rather than at the end when they hit a "submit" button, you can do so using Livewire's `update()` method hook:
...
In the above example, when a user completes a field (by clicking onto the next one) a network request is sent to update that property on the component. Immediately after the property is updated on the class, the `updated` hook is called for that specific property name and it's new value.

This way we can use that hook to update only that specific field in the database.

Additionally, because we have the `#[Rule]` attributes attached to those properties, the validation will be run before the property is updated and the `updated()` hook is called.

To learn more about the the "updated" lifecycle hook and other hooks [visit the documentation.](/docs/lifecycle-hooks)
...
For example, if a user visits an `UpdatePost` page and starts modifying the title of the post in a text input, it may be unclear to them when the title is actually being updated in the database. Especially if there is no "Save" button at the bottom.

Livewire provides a convenient utility called `wire:dirty` that allows you to toggle elements or modify classes when an inputs value diverges from the server-side component.
...
In the above code, when a user types into the input field a yellow border around it will appear. When they tab away and the network request is sent, the border will dissapear, signalling to them that the input has been persisted and is no longer "dirty".

The above example shows `wire:dirty` being used to toggle a class on the element by adding `.class` at the end of it. You can also use it to toggle HTML attributes on the element as well using `.attr`:

```html
<input type="text" wire:model="title" wire:dirty.attr="disabled">
```

If you want to toggle an entire element's visibility, you can do so by using `wire:dirty` in conjunction with `wire:target`. `wire:target` is necessary to specify which piece of data you want to watch for "dirtyness". In this case, the "title" field:
...
## Wrapping input fields in Blade components

Even in a small component such as the `CreatePost` example we've been using, we end up duplicating lots of form field code like validation messages and labels for example.
...
Let's extract the "title" input from above into a dedicated Blade component called `x-input-text` stored in the `resources/views/components` directory.

Here's how we might use this component from the `CreatePost` template:

```html
<x-input-text name="title" wire:model="title" />
```

Now here's the source for it:
...
As you can see, we took the repetitive HTML and placed it inside a dedicated Blade component file.

For the most part it's pretty straightforward except you might notice two added bits: The `@props` directive at the top and the `{{ $attributes }}` statement on the input. Let's walk through those:

By specifying `name` as a "prop" using `@props(['name'])` we are telling Blade: Hey, if an attribute called "name" is set on this component, take it's value and make it available inside this component as `$name`.
...
For other attributes that don't have an explicity purpose, we used the `{{ $attributes }}` statement. This is use for "attribute forwarding", or in other words, taking any HTML attributes written on the Blade component and forwarding them onto an element within the component, `<input>` in our case.
...
This is an extremely powerful pattern and we've only scratched the surface here. For in-depth guides on this pattern, you can [watch our series of dedicated screencasts here.]

### Custom inputs

In the previous example, we "wrapped" a native input into a nice, contained, Blade component we can use as if it's a native input element.

This will get you pretty far, however, there might be some cases where you want to create an entire input component from scratch (without an underlying native input element).

Let's say you wanted to create a `<x-input-counter />` component that was a simple "counter" component written in Alpine, except you could use it within a Livewire component as if it were a normal, native, input element, using `wire:model` and such. 

First, let's look at a simple, pure-Alpine, "counter" component:
...
For the most part, creating this component is simple. We take the HTML of the counter and place it inside a Blade component tamplate like `resources/views/components/counter.blade.php`.
...
`{{ $attributes }}`, like we explored earlier, forwards any attributes passed into the Blade component from outside, `wire:model` in our case.
...
Because `x-modelable` works for both `wire:model` and `x-model`, you can also use this Blade component interchangably with Livewire and Alpine. For example, here's an example of using this Blade component in a purely Alpine context:

```html
<x-input-counter x-model="quantity" />
```

Creating custom input elements in your application is extremely powerful, but requires a deeper understanding of the utilities Livewire and Alpine provide and how they interact with each other. For deeper information on this technique, [watch our in-depth screencasts on the subject.]
...
Here's a comprehensive list of the different avilable input types and how you use them in a Livewire context.
...
Checkboxes can be used for either single values, for example when toggling a boolean property, or for toggling a single value in a group of related values. We'll cover both scenarios:

#### Single checkbox

At the end of a signup form, you might have a checkbox allowing the user to opt-into email updates. You might call this property "receiveUpdates" internally. You can easily bind this value to the checkbox using `wire:model` like so:
...
Now, lets's say in addition to deciding to receive updates, you have an array property in your class called `updateTypes` like so:

```php
public $updateTypes = [];
```

You can allow the user to chose any of the following update types and they will be added to the `$updateTypes` property on the component:
