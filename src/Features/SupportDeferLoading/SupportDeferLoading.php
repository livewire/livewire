...
namespace Livewire\Features\SupportDeferLoading;
...
class SupportDeferLoading extends ComponentHook
{
    public function mount($params)
    {
        if (($params['lazy'] ?? false) || !($params['defer'] ?? false)) return;

        $this->component->skipMount();

        $mountParams = array_diff_key($params, array_flip(['defer']));
    ...
    {
        if (isset($memo['deferLoaded'])) return;

        if ($memo['deferLoaded'] === false) $this->component->skipHydrate();
    }

    function dehydrate($context)
    {
        $context->addMemo('deferLoaded', false);

        if (!$context->mounting) return;

        $context->addMemo('deferLoaded', true);
    }

    function call($method, $params, $returnEarly)
    {
        if ($method !== '__deferLoad') return;
    ...
    public function generatePlaceholderHtml($params)
    {
        ...
        $html = Utils::insertAttributesIntoHtmlRoot($placeholder, [
            'x-init' => '$wire.__deferLoad(\'' . $encoded . '\')',
        ]);
