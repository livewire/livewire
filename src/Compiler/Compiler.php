...
use Livewire\V4\Compiler\ParsedComponent;
...
use Livewire\V4\Compiler\CompilationResult;
use Livewire\Mechanisms\Mechanism;
use Illuminate\Support\Facades\File;

class Compiler extends Mechanism
{
    ...
    public function compile(string $path): CompilationResult
    {
        if (! file_exists($path)) {
            throw new CompilationException("View file not found: [{$path}]");
        }

        // Need to compile - read content only now
        $content = File::get($path);

        $sourceModTime = filemtime($path);

        $hash = $this->generateHash($path, $sourceModTime);

        // Parse the component...
        $parsed = $this->parseComponent($content);
        $parsed = $this->loadExternalViewAndScriptIfRequired($path, $parsed);

        // Generate compilation result...
        $result = $this->generateCompilationResult($path, $parsed, $hash);

        // Compile islands...
        $islandsCompiler = new IslandsCompiler($this->cacheDirectory);
        $parsed->viewContent = $islandsCompiler->compile($parsed->viewContent, $path);

        // Compile placeholder...
        $placeholderCompiler = new PlaceholderCompiler($this->cacheDirectory);
        $parsed->viewContent = $placeholderCompiler->compile($parsed->viewContent, $path);

        // Generate files and metadata...
        $this->generateFiles($result, $parsed);
        $this->storeCompilationMetadata($result, $parsed, [$path], [$sourceModTime]);
