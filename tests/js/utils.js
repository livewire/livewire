...
export function mount(dom, requestInterceptor = () => {}) {
    document.body.innerHTML = `<div wire:id="123" wire:initial-data="${e(
        JSON.stringify({
            fingerprint: { id: 123, name: 'foo', locale: 'enUS' },
            effects: {},
            serverMemo: { data },
        })
    )}">${dom}</div>`

    window.__testing_request_interceptor = payload =>
...
export function mountAsRootAndReturn(
    dom,
    returnedDom,
    dirtyInputs = [],
...
export function mountWithEvent(dom, event, requestInterceptor = () => {}) {
    document.body.innerHTML =
        '<div wire:id="123" wire:initial-data="{&quot;events&quot;:[&quot;' +
        event +
        '&quot;]}">' +
        dom +
        '</div>'
...
export function mountAndReturn(
    dom,
    returnedDom,
    dirtyInputs = [],
    requestInterceptor = async () => {}
    ...
    window.__testing_request_interceptor = async (payload, connection) => {
        await requestInterceptor(payload)
        setTimeout(() => {
            connection.onMessage(window.livewire.first().messageInTransit, {
                effects: {
                    html: '<div wire:id="123">' + returnedDom + '</div>',
                },
                serverMemo: {
                    data: {},
                    dirty: dirtyInputs,
...
export function mountAndReturnEmittedEvent(
    dom,
    event,
    requestInterceptor = async () => {}
) {
    ...
    document.body.innerHTML = ''
    document.body.innerHTML =
        '<div wire:id="123" wire:initial-data=""  wire:events="[]">' +
        dom +
        '</div>'
...
export function mountAndReturnDispatchedEvent(
    dom,
    event,
    requestInterceptor = async () => {}
) {
...
export function mountAndError(dom, requestInterceptor = async () => {}) {
    ...
    document.body.innerHTML =
        '<div wire:id="123" wire:initial-data="">' + dom + '</div>'
...
export function mountAndReturnWithData(
    dom,
    returnedDom,
    data,
    dirtyInputs = []
