...
test('receive event from global fire', async () => {
    ...
    await wait(() => {
        expect(payload.updateQueue[0].type).toEqual('fireEvent')
        expect(payload.updateQueue[0].payload.event).toEqual('foo')
        expect(payload.updateQueue[0].payload.params).toEqual(['bar'])
    })
})
...
test('receive event from action fire', async () => {
    var payload
    mountWithEvent(
        "<button wire:click=\"$emit('foo', 'bar')\"></button>",
        ['foo'],
        i => (payload = i)
    )
...
test('receive event from component fired only to ancestors, and make sure global listener doesnt receive it', async () => {
    ...
    await wait(() => {
        expect(payloadsThatSentARequest.length).toEqual(2)
    })
...
test('receive event from action fired only to ancestors, and make sure global listener doesnt receive it', async () => {
    ...
    await wait(() => {
        expect(payloadsThatSentARequest.length).toEqual(1)
    })
...
describe('test Laravel Echo', () => {
    ...
    test('public echo channel is created and reacts', async () => {
        ...
        await wait(() => {
            expect(payload.updateQueue[0].type).toEqual('fireEvent')
            expect(payload.updateQueue[0].payload.event).toEqual('echo:foo,bar')
            expect(payload.updateQueue[0].payload.params).toEqual(['baz'])
        })
    })
    ...
    test('private echo channel is created and reacts', async () => {
        ...
        await wait(() => {
            expect(payload.updateQueue[0].type).toEqual('fireEvent')
            expect(payload.updateQueue[0].payload.event).toEqual(
                'echo-private:foo,bar'
            )
            expect(payload.updateQueue[0].payload.params).toEqual(['baz'])
        })
