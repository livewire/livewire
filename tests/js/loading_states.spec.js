...
test('loading is scoped to current element if it fires an action', async () => {
    ...
    expect(document.querySelector('span').classList.contains('baz')).toEqual(
        false
    )
    ...
    await wait(() => {
        ...
        expect(
            document.querySelector('span').classList.contains('baz')
        ).toEqual(false)
    })
...
test('loading is scoped to current element if it binds data with wire:model', async () => {
    ...
    expect(document.querySelector('button').disabled).toEqual(false)
    expect(document.querySelector('input').classList.contains('baz')).toEqual(
        false
    )
    ...
    await wait(() => {
        expect(document.querySelector('button').disabled).toEqual(true)
        expect(
            document.querySelector('input').classList.contains('baz')
        ).toEqual(false)
    })
...
test('add element class with spaces while loading', async () => {
    mount(
        '<button wire:click="onClick"></button><span wire:loading.class="foo bar"></span>'
    )
    ...
    await wait(() => {
        expect(
            document.querySelector('span').classList.contains('foo')
        ).toBeTruthy()
        expect(
            document.querySelector('span').classList.contains('bar')
        ).toBeTruthy()
    })
})
...
test('remove element class while loading', async () => {
    mount(
        '<button wire:click="onClick"></button><span class="hidden" wire:loading.class.remove="hidden"></span>'
    )
    ...
    await wait(() => {
        expect(
            document.querySelector('span').classList.contains('hidden')
        ).toBeFalsy()
    })
})
...
test('add element attribute while loading', async () => {
    mount(
        '<button wire:click="onClick"></button><span wire:loading.attr="disabled"></span>'
    )
    ...
    await wait(() => {
        expect(
            document.querySelector('span').hasAttribute('disabled')
        ).toBeTruthy()
    })
})
...
test('remove element attribute while loading', async () => {
    mount(
        '<button wire:click="onClick"></button><span wire:loading.attr.remove="disabled" disabled="true"></span>'
    )
...
test('add element attribute AND class while loading', async () => {
    mount(
        '<button wire:click="onClick"></button><span wire:loading.attr="disabled" wire:loading.class="foo"></span>'
    )
...
test('do nothing when the loading class is empty when adding classes', async () => {
    mount(
        '<button wire:click="onClick"></button><span wire:loading.class=""></span>'
    )
...
test('do nothing when the loading class is empty when removing classes', async () => {
    mount(
        '<button wire:click="onClick"></button><span class="foo" wire:loading.remove.class=""></span>'
    )
