<?php

namespace Tests\Browser\Morph;

use Livewire\Component as BaseComponent;

class WithAlpineComponent extends BaseComponent
{
    public $showPreviousChild = false;

    public function render()
    {
        return <<< 'HTML'
<div>
    <button wire:click="$toggle('showPreviousChild')" dusk="togglePreviousChild">Toggle Previous Child</button>

    <div dusk="output">
        @if ($showPreviousChild)
            <div>first</div>
        @endif

        <div>
            <div>second</div>
            <div wire:key="alpine" x-data="{ show: false }">
                <span x-text="show" dusk="alpineOutput"></span>
                <button x-on:click="show = !show" dusk="toggleAlpine">Toggle Alpine</button>
            </div>
        </div>
    </div>

    <script>window.useMorphdom = true</script>
</div>
HTML;
    }
}
